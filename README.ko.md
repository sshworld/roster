...
roster 는 세 개의 명령(`audit`, `doccheck`, `usage`)을 가진 단일 바이너리입니다. 플러그인은 `audit` 과 `usage` 를 `/roster-audit`·`/roster-usage` 스킬로 감싸고, 둘 위에 얹은 스킬 전용 대화형 워크플로 `/roster-cleanup` 을 추가합니다. `doccheck` 은 CLI 전용입니다.
